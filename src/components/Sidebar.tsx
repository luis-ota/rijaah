import { Layers, LayoutDashboard, List, GitBranch, Settings, Plus, ChevronDown, ChevronRight, LogOut, Calendar, TrendingDown, BarChart3, Tag } from 'lucide-react';
...
export default function Sidebar({
  ...
  const navItems: { view: View; icon: React.ReactNode; label: string }[] = [
    ...
    { view: 'burndown', icon: <TrendingDown className="w-4 h-4" />, label: 'Burndown' },
    { view: 'report', icon: <BarChart3 className="w-4 h-4" />, label: 'Report' },
    { view: 'labels', icon: <Tag className="w-4 h-4" />, label: 'Labels' },
  ...
  return (
    <aside className="w-60 bg-slate-900 text-white flex flex-col h-screen sticky top-0 shrink-0">
      ...
      <div className="flex items-center gap-2.5 px-4 py-4 border-b border-slate-700/50">
        ...
        <span className="font-semibold text-sm text-white">Rijaah</span>
      </div>
      ...
      <div
        ...
        onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') setProjectsExpanded(v => !v); }}
        className="flex items-center justify-between w-full text-xs font-semibold text-slate-400 uppercase tracking-wider px-1 py-1 hover:text-slate-300 transition-colors cursor-pointer"
      >
        <span>Projects</span>
